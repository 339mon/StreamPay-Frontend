...
### Added
- Prometheus metrics on `GET|POST /api/exports`: `export_requests_total`
  (counter) and `export_request_duration_seconds` (histogram), labeled by
  `status` and `method`, registered in `src/metrics/registry.ts`.
